...
The per-dataset files retain their original report schema 2 evidence. The
catalogue summarizer classifies those unchanged records as `complete_pass`,
`empty_neural_data`, or a specific failure category. The current schema 4
verifier adds optional on-disk MATLAB round-trip fields; historical reports are
not rewritten to pretend those later checks ran at collection time.
